...
export default function ProjectList(props) {
   ...
   const userLogin = { ...JSON.parse(localStorage.getItem(USER_LOGIN_LOCAL_STORAGE)) };
   ...
   const columns = [
      {
         ...
         render: (text, record, index) => {
            return (
               <NavLink to={`/project/board/${record.id}`} style={{ cursor: "pointer" }}>
                  {text}
               </NavLink>
            );
         },
      ...
      },
      {
         ...
         ellipsis: true,
         render: (text, record, index) => {
            return record.createdBy === "ADMIN" ? (
               <Tag color="#f50" key={index}>
                  {record.createdBy}
               </Tag>
            ) : record.createdBy === "Member" ? (
      ...
      },
      {
         ...
         render: (text, record, index) => (
            <div style={{ display: "flex" }}>
               ...
               {userLogin.login === record.createdBy ? (
                  <div>
                     <span>
                        <Popconfirm
