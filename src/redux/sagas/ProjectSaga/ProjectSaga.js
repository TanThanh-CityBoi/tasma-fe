...
function* updateProjectSaga(action) {
  try {
    ...
    if (status === STATUS_CODE.SUCCESS) {
      if (
        data.status === 400 &&
        data.message === "Project name already exist!"
      ) {
...
function* addMemberToProjectSaga(action) {
  try {
    const { data, status } = yield call(() =>
      projectService.addMember(action.project)
    );
...
function* deleteMemberFromProjectSaga(action) {
  try {
    const { data, status } = yield call(() =>
      projectService.deleteMember(action.project)
    );
...
function* getListMemberSaga(action) {
  try {
    ...
    const { data, status } = yield call(() =>
      projectService.getListMembers(projectId)
    );
